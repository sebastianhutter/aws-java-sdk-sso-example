...
public class App {
     ...
     */
    private static ClientInfo registerClient(AWSSSOOIDC oidcClient, String issuerUrl) {
        System.out.println("Registering client...");

        RegisterClientRequest registerRequest = new RegisterClientRequest()
                .withClientName("my-java-app-refresh-token")
                .withClientType("public")
                .withIssuerUrl(issuerUrl)
                .withGrantTypes("authorization_code", "refresh_token")
                .withScopes("sso:account:access")
                .withRedirectUris(REDIRECT_URI);
        ...
        return new ClientInfo(registerResponse.getClientId(), registerResponse.getClientSecret());
    }
    ...
    private static TokenInfo authenticateWithAuthorizationCode(
        ...
        try {
            ...
            CreateTokenRequest tokenRequest = new CreateTokenRequest()
                    ...
                    .withClientSecret(clientInfo.clientSecret)
    ...
    private static TokenInfo refreshAccessToken(
        ...
        return new TokenInfo(
                tokenResponse.getAccessToken(),
                tokenResponse.getRefreshToken(), // May be null if server doesn't rotate refresh tokens
                tokenResponse.getExpiresIn());
    }
    ...
    public static void listS3Buckets(String region, RoleCredentials credentials) {
        ...
        try {
            ...
            System.out.println("S3 Buckets (count: " + buckets.size() + "):");
            buckets.stream().limit(5).forEach(bucket ->
                System.out.println("  - " + bucket.getName()));
            if (buckets.size() > 5) {
                System.out.println("  ... and " + (buckets.size() - 5) + " more");
            }
        } catch (Exception e) {
